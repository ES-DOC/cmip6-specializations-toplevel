...
<html lang="en">
    ...
    <header class="clearfix main">
        ...
        <h1 class="pull-right">
            ...
            <button class="short-table-toggle btn btn-info">Short Tables</button>
        </h1>
    </header>
    </body>
</html>
...
class Generator(SpecializationParser):
    ...
    def on_property_parse(self, prop):
        ...
        if prop.enum:
            ...
            html = _HTML_PROPERTY.format(
                'short-table' if self.is_short_table_member(prop) else '',
                prop.long_name,
                prop.cardinality,
                enum)
        else:
    ...
    def is_short_table_member(self, prop):
        for short_table in self.short_tables:
            for group in short_table.groups:
                if prop.id in group.identifiers:
                    return True
        return False
